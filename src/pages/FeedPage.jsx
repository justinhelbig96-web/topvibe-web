...
export default function FeedPage() {
  ...
  const onMouseMove = (e) => { if (!dragging) return; setDragX(e.clientX - dragStartX.current); };
  const onMouseUp = () => {
    if (!dragging) return;
    setDragging(false);
  ...
  const onTouchMove = (e) => { setDragX(e.touches[0].clientX - dragStartX.current); };
  const onTouchEnd = () => {
    if (dragX > 80) vote('fire');
    else if (dragX < -80) vote('skip');
    setDragX(0);
  ...
  if (!track) return (
    <div className="feed-empty">
      ...
      <div className="feed-empty__genres">
        {GENRES.map(g => (
          <button key={g.id} className={`genre-tab ${genre === g.id ? 'active' : ''}`} onClick={() => setGenre(g.id)}>
            {g.label}
          </button>
        ))}
      </div>
      ...
      <div className="feed-main">
        ...
        <div className="card-area">
          ...
          <div
            ...
            style={{ transform: `translateX(${dragX}px) rotate(${rotation}deg)`, cursor: dragging ? 'grabbing' : 'grab' }}
          ...
          >
            <img src={track.album?.images?.[0]?.url} alt={track.name} className="card-cover" draggable={false} />
            ...
            <div className="card-bottom-gradient">
              <div className="card-track-info">
                ...
                <p className="card-track-artist">{track.artists?.map(a => a.name).join(', ')}</p>
              </div>
            </div>
            <div className="card-stats">
              <span className="stat-fire">🔥 {stats.fireCount}</span>
              <div className="stat-bar"><div className="stat-bar-fill" style={{ width: firePercent + '%' }} /></div>
        ...
        <div className="feed-info">
          <div className="feed-info__header">
            <div>
            ...
            <span className="feed-track-counter">{index + 1} / {tracks.length}</span>
          </div>

          {/* Player */}
          {previewUrl ? (
            <div className="feed-player">
              <button className="feed-play-btn" onClick={togglePlay}>
                {isPlaying
                  ? <svg viewBox="0 0 24 24" fill="currentColor"><rect x="6" y="4" width="4" height="16"/><rect x="14" y="4" width="4" height="16"/></svg>
                  : <svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z"/></svg>
                }
              </button>
              <div className="feed-player__right">
                <div className="feed-player__label">{isPlaying ? 'Playing preview' : '30s preview'}</div>
          ...
          <div className="feed-stats-card">
            <div className="feed-stats-row">
              <div className="feed-stat">
                <span className="feed-stat__icon">🔥</span>
                <span className="feed-stat__num">{stats.fireCount}</span>
                <span className="feed-stat__label">Fire</span>
              </div>
              <div className="feed-stat-divider" />
              <div className="feed-stat">
                <span className="feed-stat__icon">💀</span>
                <span className="feed-stat__num">{stats.skipCount}</span>
                <span className="feed-stat__label">Skip</span>
              </div>
              {totalVotes > 0 && (
                <>
                  <div className="feed-stat-divider" />
                  <div className="feed-stat">
                    <span className="feed-stat__icon" style={{ color: 'var(--green)', fontWeight: 800 }}>{firePercent}%</span>
                    <span className="feed-stat__label">fire rate</span>
                  </div>
                </>
              )}
            </div>
          ...
          <div className="vote-buttons">
            ...
            <button className="vote-btn fire-btn" onClick={() => vote('fire')}>
              <svg viewBox="0 0 24 24" fill="currentColor"><path d="M13.5 0.67s.74 2.65.74 4.8c0 2.06-1.35 3.73-3.41 3.73-2.07 0-3.63-1.67-3.63-3.73l.03-.36C5.21 7.51 4 10.62 4 14c0 4.42 3.58 8 8 8s8-3.58 8-8C20 8.61 17.41 3.8 13.5.67z"/></svg>
              Fire 🔥
            </button>
          </div>

          <p className="swipe-hint">← Drag to skip · Drag to fire →</p>
        </div>
      </div>
    </div>
  );
