...
function ChevronIcon({ open }) {
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={`section-chevron ${open ? 'open' : ''}`}>
          ...
          <div className="profile-card">
            ...
            <div className="profile-stats-row">
              <div className="profile-stat">
                <span className="profile-stat__num">{topTracks.length}</span>
                <span className="profile-stat__label">Top tracks</span>
              </div>
              <div className="profile-stat">
                <span className="profile-stat__num">{likedTracks.length}</span>
                <span className="profile-stat__label">Fire votes</span>
              </div>
            </div>
        ...
        <div className="profile-content">
          {topTracks.length > 0 && (
            <div className="profile-section">
              <button className="section-toggle" onClick={() => setShowTopTracks(v => !v)}>
                <h2>Your Top Tracks</h2>
                <ChevronIcon open={showTopTracks} />
              </button>
              {showTopTracks && (
          ...
          {likedTracks.length > 0 && (
            <div className="profile-section">
              <button className="section-toggle" onClick={() => setShowFireVotes(v => !v)}>
                <h2>Your Fire Votes 🔥</h2>
                <ChevronIcon open={showFireVotes} />
              </button>
              {showFireVotes && (
                <div className="track-list">
                  {likedTracks.map((t) => (
          ...
          {topTracks.length === 0 && likedTracks.length === 0 && (
            <div className="profile-empty">
              <p>Start voting to see your stats here!</p>
            </div>
          )}
        </div>
      </div>
